...
import { Calendar, CheckCircle, ShieldCheck, HelpCircle, ChevronRight, Phone } from 'lucide-react';
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center bg-white rounded-3xl p-8 sm:p-12 border border-slate-100 shadow-sm">
          <div className="lg:col-span-7 space-y-4">
            ...
            <div className="pt-4 flex flex-wrap items-center gap-4">
              <Link to={`/appointment?service=${encodeURIComponent(service.name)}`} className="px-8 py-3.5 rounded-full text-sm font-bold text-white bg-gradient-to-r from-[#0E5D94] to-[#0A4D7A] hover:from-[#0a4975] hover:to-[#0E5D94] shadow-md transition-all flex items-center gap-2">
                <Calendar className="w-4 h-4 text-[#E83E8C]"/>
                <span>Book This Treatment</span>
              </Link>

              <a href={`tel:${settings.phone.replace(/\s+/g, '')}`} className="px-6 py-3.5 rounded-full text-sm font-bold text-slate-700 bg-slate-100 hover:bg-slate-200 transition-colors flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-8 space-y-12">
            ...
            <div className="bg-white rounded-2xl p-8 border border-slate-100 shadow-xs space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                {service.benefits.map((b, idx) => (<div key={idx} className="flex items-start gap-3 p-3.5 rounded-xl bg-sky-50/50 border border-sky-100">
            ...
            {/* Procedure Steps */}
            {service.process && service.process.length > 0 && (<div className="bg-white rounded-2xl p-8 border border-slate-100 shadow-xs space-y-6">
                <h2 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
                  <span className="w-2.5 h-6 rounded-sm bg-[#0E5D94]"></span>
                  Step-By-Step Procedure Protocol
                </h2>
                <div className="space-y-6">
                  {service.process.map((step) => (<div key={step.stepNumber} className="flex items-start gap-4 pb-6 border-b border-slate-100 last:border-0 last:pb-0">
            ...
            {/* Service FAQs */}
            {service.faqs && service.faqs.length > 0 && (<div className="bg-white rounded-2xl p-8 border border-slate-100 shadow-xs space-y-6">
                <h2 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
                  <HelpCircle className="w-6 h-6 text-[#E83E8C]"/>
                  Treatment Specific FAQs
                </h2>
                <div className="space-y-4">
                  {service.faqs.map((faq, i) => (<div key={i} className="p-4 rounded-xl bg-slate-50 border border-slate-100 space-y-2">
          ...
          <div className="lg:col-span-4 space-y-8">
            ...
            <div className="bg-gradient-prisha text-white rounded-2xl p-6 shadow-lg space-y-4">
              ...
              <Link to={`/appointment?service=${encodeURIComponent(service.name)}`} className="w-full block text-center py-3 rounded-xl text-xs font-bold text-[#0E5D94] bg-white hover:bg-sky-50 transition-colors shadow-sm">
            ...
            <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-xs space-y-4">
              ...
              <ul className="space-y-2.5 text-xs">
                {otherServices.map((os) => (<li key={os.slug}>
                    <Link to={`/services/${os.slug}`} className="flex items-center justify-between p-2 rounded-lg hover:bg-sky-50 text-slate-700 hover:text-[#0E5D94] transition-colors">
