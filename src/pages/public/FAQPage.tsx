      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-8 space-y-12">
        ...
        <div className="bg-white p-4 rounded-2xl shadow-xs border border-slate-100 space-y-3">
          <div className="flex items-center gap-2 px-3.5 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus-within:bg-white focus-within:border-[#0E5D94] transition-colors">
            ...
            <input
              type="text"
              placeholder="Search by topic, e.g., ICSI, pain, blastocyst, cost..."
              value={searchQuery}
        ...
        <div className="space-y-4">
          {filteredFaqs.map((faq) => {
            ...
            return (
              <div
                key={faq.id}
                className="bg-white rounded-2xl border border-slate-200/70 overflow-hidden shadow-xs transition-all"
              >
                <button
                  onClick={() => setExpandedId(isOpen ? null : faq.id)}
                  className="w-full text-left p-5 sm:p-6 flex items-center justify-between gap-4 font-bold text-slate-900 hover:text-[#0E5D94] transition-colors"
                >
                ...
                {isOpen && (
                  <div className="p-5 sm:p-6 pt-0 text-xs sm:text-sm text-slate-600 leading-relaxed border-t border-slate-50">
                    {faq.answer}
                  </div>
                )}
              </div>
        ...
        <div className="bg-white rounded-3xl p-8 border border-slate-100 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-3">
            <a
              ...
              className="px-5 py-2.5 rounded-full text-xs font-bold text-[#0E5D94] bg-sky-50 hover:bg-sky-100 border border-sky-200 transition-colors flex items-center gap-1.5"
            >
            ...
            <Link
              to="/appointment"
              className="px-5 py-2.5 rounded-full text-xs font-bold text-white bg-[#0E5D94] hover:bg-[#0a4975] transition-colors flex items-center gap-1.5 shadow-xs"
            >
