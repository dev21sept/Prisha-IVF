    ...
    return (<div className="w-full bg-[#f8fbfe] py-12 lg:py-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-8 space-y-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {testimonials.map((item) => (<div key={item.id} className="bg-white rounded-2xl p-8 border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col justify-between">
              <div>
              ...
              <div className="pt-4 border-t border-slate-100 flex items-center gap-4">
                <img src={item.image} alt={item.patientName} className="w-12 h-12 rounded-full object-cover border-2 border-[#0E5D94] shadow-xs"/>
                <div>
        ...
        <div className="bg-gradient-prisha text-white rounded-3xl p-10 text-center space-y-4 shadow-lg">
          ...
          <div className="pt-2">
            <Link to="/appointment" className="inline-flex items-center gap-2 px-8 py-3.5 rounded-full text-xs font-bold text-[#0E5D94] bg-white hover:bg-sky-50 transition-colors shadow-sm">
