...
import { Calendar, ArrowRight, Award, Microscope, Activity, HeartHandshake, Sparkles, Phone, Clock, MapPin, ChevronRight, ChevronLeft, Star, Plus, Minus, Check, Volume2, VolumeX, Heart } from 'lucide-react';
export const HomePage = () => {
    ...
    return (<div className="w-full bg-[#f8fbfe] overflow-hidden">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
            ...
            <div className="lg:col-span-8 space-y-6 animate-slide-up" key={activeSlide.id}>
              ...
              <div className="flex flex-wrap gap-4 pt-1">
                {activeSlide.points.map((pt, i) => (<div key={i} className="flex items-center gap-2 text-xs font-semibold text-sky-100 bg-white/10 px-3 py-1.5 rounded-lg backdrop-blur-xs border border-white/10">
              ...
              <div className="flex flex-wrap items-center gap-4 pt-4">
                <Link to={activeSlide.primaryBtn.link} className="px-8 py-4 rounded-full text-sm font-bold text-white bg-gradient-to-r from-[#E83E8C] via-[#d81b60] to-[#b81b62] hover:opacity-95 shadow-xl transition-all duration-300 flex items-center gap-2 group btn-shimmer">
                ...
                <Link to={activeSlide.secondaryBtn.link} className="px-7 py-4 rounded-full text-sm font-bold text-white bg-white/15 hover:bg-white/25 border border-white/30 backdrop-blur-md transition-all duration-300 shadow-md">
          ...
          <div className="pt-12 flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2.5">
                {heroSlides.map((slide, idx) => (<button key={slide.id} onClick={() => setCurrentSlide(idx)} className={`h-2 rounded-full transition-all duration-300 ${currentSlide === idx ? 'w-10 bg-[#E83E8C]' : 'w-2.5 bg-white/40 hover:bg-white/70'}`} aria-label={`Go to slide ${idx + 1}`}/>))}
              </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={toggleVideoMute} className="inline-flex items-center gap-2 px-3.5 py-2 rounded-full bg-black/30 hover:bg-black/50 text-white text-xs font-semibold backdrop-blur-md border border-white/20 transition-all cursor-pointer" title={isVideoMuted ? "Unmute Video Audio" : "Mute Video Audio"}>
                {isVideoMuted ? (<>
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => setCurrentSlide((prev) => (prev - 1 + heroSlides.length) % heroSlides.length)} className="w-10 h-10 rounded-full bg-black/30 hover:bg-black/50 text-white flex items-center justify-center backdrop-blur-md border border-white/20 transition-colors cursor-pointer" aria-label="Previous Slide">
                  <ChevronLeft className="w-5 h-5"/>
                </button>
                <button onClick={() => setCurrentSlide((prev) => (prev + 1) % heroSlides.length)} className="w-10 h-10 rounded-full bg-black/30 hover:bg-black/50 text-white flex items-center justify-center backdrop-blur-md border border-white/20 transition-colors cursor-pointer" aria-label="Next Slide">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            ...
            <div className="lg:col-span-6 relative">
              <div className="relative rounded-3xl overflow-hidden shadow-2xl border-4 border-white">
                <img src={homepage.aboutImageUrl || 'https://images.unsplash.com/photo-1579684385127-1ef15d508118?auto=format&fit=crop&w=800&q=80'} alt="Prisha IVF Medical Facility" className="w-full h-[450px] object-cover hover:scale-105 transition-transform duration-700"/>
            ...
            <div className="lg:col-span-6 space-y-6">
              ...
              <div className="space-y-3 pt-2">
                {homepage.aboutFeatures.map((feat, idx) => (<div key={idx} className="flex items-start gap-3">
              ...
              <div className="pt-4 flex items-center gap-5">
                <Link to="/about" className="px-7 py-3.5 rounded-full text-sm font-bold text-white bg-[#0E5D94] hover:bg-[#0a4975] transition-all shadow-md flex items-center gap-2">
      ...
      <section className="py-20 lg:py-28 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {featuredServices.map((srv) => (<div key={srv.slug} className="group bg-white rounded-2xl border border-slate-100 p-6 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col justify-between fertiora-card hover:border-sky-200">
                <div>
                  ...
                  <ul className="space-y-1.5 mb-6 text-xs text-slate-600">
                    {srv.benefits.slice(0, 2).map((b, i) => (<li key={i} className="flex items-center gap-1.5">
                ...
                <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
                  <Link to={`/services/${srv.slug}`} className="text-xs font-bold text-[#0E5D94] group-hover:text-[#E83E8C] flex items-center gap-1 transition-colors">
          ...
          <div className="mt-12 text-center">
            <Link to="/services" className="inline-flex items-center gap-2 px-8 py-3.5 rounded-full text-sm font-bold text-[#0E5D94] bg-sky-50 hover:bg-sky-100 border border-sky-200 transition-colors">
      ...
      <section className="py-20 lg:py-28 bg-[#f8fbfe] border-y border-slate-200/60">
        <div className="max-w-7xl mx-auto px-4 sm:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {homepage.whyChooseItems.map((item, idx) => (<div key={idx} className="bg-white rounded-2xl p-7 shadow-sm hover:shadow-lg border border-slate-100 transition-all duration-300 relative group fertiora-card">
      ...
      <section className="py-20 lg:py-28 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 relative">
            {homepage.treatmentProcessSteps.map((step, idx) => (<div key={idx} className="relative bg-sky-50/40 rounded-2xl p-7 border border-sky-100 hover:bg-white hover:shadow-xl transition-all duration-300 fertiora-card">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            ...
            <div className="lg:col-span-6">
              <div className="bg-white rounded-3xl p-6 sm:p-10 shadow-2xl text-slate-800 border border-slate-100">
                {bookingSuccess ? (<div className="py-8 text-center space-y-4">
                    ...
                    <button onClick={() => setBookingSuccess(null)} className="mt-4 px-6 py-2.5 text-xs font-bold text-[#0E5D94] bg-sky-50 rounded-xl hover:bg-sky-100 transition-colors">
                      Book Another Appointment
                    </button>
                  </div>) : (<form onSubmit={handleBookSubmit} className="space-y-4 text-xs">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div>
                        ...
                        <input type="text" required placeholder="e.g. Ananya Roy" value={patientName} onChange={(e) => setPatientName(e.target.value)} className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-xs focus:border-[#0E5D94] focus:ring-2 focus:ring-sky-100 outline-none transition-all"/>
                      </div>

                      <div>
                        <label className="block font-bold text-slate-700 mb-1">Mobile / WhatsApp *</label>
                        <input type="tel" required placeholder="+91 98765 43210" value={patientPhone} onChange={(e) => setPatientPhone(e.target.value)} className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-xs focus:border-[#0E5D94] focus:ring-2 focus:ring-sky-100 outline-none transition-all"/>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div>
                        ...
                        <input type="email" placeholder="ananya@example.com" value={patientEmail} onChange={(e) => setPatientEmail(e.target.value)} className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-xs focus:border-[#0E5D94] focus:ring-2 focus:ring-sky-100 outline-none transition-all"/>
                      </div>

                      <div>
                        <label className="block font-bold text-slate-700 mb-1">City / Location</label>
                        <input type="text" placeholder="e.g. New Delhi / Gurugram" value={patientCity} onChange={(e) => setPatientCity(e.target.value)} className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-xs focus:border-[#0E5D94] focus:ring-2 focus:ring-sky-100 outline-none transition-all"/>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div>
                        ...
                        <select value={selectedDoctor} onChange={(e) => setSelectedDoctor(e.target.value)} className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-xs focus:border-[#0E5D94] outline-none bg-white">
                      ...
                      <div>
                        ...
                        <select value={selectedService} onChange={(e) => setSelectedService(e.target.value)} className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-xs focus:border-[#0E5D94] outline-none bg-white">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div>
                        ...
                        <input type="date" required value={preferredDate} onChange={(e) => setPreferredDate(e.target.value)} className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-xs focus:border-[#0E5D94] outline-none"/>
                      </div>

                      <div>
                        <label className="block font-bold text-slate-700 mb-1">Preferred Slot</label>
                        <select value={preferredTime} onChange={(e) => setPreferredTime(e.target.value)} className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-xs focus:border-[#0E5D94] outline-none bg-white">
                    ...
                    <div>
                      ...
                      <textarea rows={2} placeholder="Tell us about previous attempts or questions..." value={patientMessage} onChange={(e) => setPatientMessage(e.target.value)} className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-xs focus:border-[#0E5D94] outline-none"/>
                    </div>

                    <button type="submit" disabled={bookingLoading} className="w-full py-3.5 rounded-xl font-bold text-white bg-gradient-to-r from-[#E83E8C] to-[#d81b60] hover:from-[#d81b60] hover:to-[#b81b62] shadow-md hover:shadow-lg transition-all disabled:opacity-50 text-sm">
      ...
      <section className="py-20 lg:py-28 bg-[#f8fbfe]">
        <div className="max-w-7xl mx-auto px-4 sm:px-8">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-16">
            ...
            <Link to="/doctors" className="inline-flex items-center gap-2 px-6 py-3 rounded-full text-xs font-bold text-[#0E5D94] bg-white border border-sky-200 hover:bg-sky-50 transition-colors shrink-0 shadow-xs">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {featuredDoctors.map((doc) => (<div key={doc.id} className="bg-white rounded-2xl overflow-hidden border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col justify-between group fertiora-card">
                <div>
                ...
                <div className="p-5 pt-0">
                  <Link to={`/doctors/${doc.slug}`} className="w-full block text-center py-2.5 rounded-lg text-xs font-bold text-[#0E5D94] bg-sky-50 hover:bg-sky-100 group-hover:bg-[#0E5D94] group-hover:text-white transition-colors">
      ...
      <section className="py-20 lg:py-28 bg-white border-b border-slate-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {featuredTestimonials.map((t) => (<div key={t.id} className="bg-slate-50/70 rounded-2xl p-7 border border-slate-100 shadow-sm hover:shadow-lg transition-all duration-300 flex flex-col justify-between fertiora-card">
                <div>
                ...
                <div className="pt-4 border-t border-slate-200/70 flex items-center gap-3.5">
                  <img src={t.image} alt={t.patientName} className="w-11 h-11 rounded-full object-cover border-2 border-white shadow-xs"/>
                  <div>
          ...
          <div className="mt-12 text-center">
            <Link to="/testimonials" className="inline-flex items-center gap-2 text-sm font-bold text-[#0E5D94] hover:text-[#E83E8C] transition-colors">
      ...
      <section className="py-20 lg:py-28 bg-[#f8fbfe]">
        <div className="max-w-4xl mx-auto px-4 sm:px-8">
          ...
          <div className="space-y-4">
            ...
            return (<div key={faq.id} className="bg-white rounded-xl border border-slate-200/80 overflow-hidden shadow-xs transition-all">
                  <button onClick={() => setExpandedFaq(isOpen ? null : faq.id)} className="w-full text-left p-5 flex items-center justify-between gap-4 font-bold text-slate-900 hover:text-[#0E5D94] transition-colors">
                  ...
                  {isOpen && (<div className="p-5 pt-0 text-xs sm:text-sm text-slate-600 leading-relaxed border-t border-slate-50">
                      {faq.answer}
                    </div>)}
                </div>);
        })}
          ...
          <div className="mt-10 text-center">
            <Link to="/faq" className="inline-flex items-center gap-2 px-6 py-2.5 rounded-full text-xs font-bold text-[#0E5D94] bg-white border border-sky-200 hover:bg-sky-50 shadow-xs transition-colors">
      ...
      <section className="py-20 lg:py-28 bg-white border-t border-slate-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-8">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-16">
            ...
            <Link to="/blog" className="inline-flex items-center gap-2 px-6 py-3 rounded-full text-xs font-bold text-[#0E5D94] bg-sky-50 hover:bg-sky-100 transition-colors shrink-0">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {featuredBlogs.map((b) => (<article key={b.id} className="bg-white rounded-2xl overflow-hidden border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col justify-between group fertiora-card">
                <div>
                ...
                <div className="p-6 pt-0 border-t border-slate-50 flex items-center justify-between text-xs">
                  ...
                  <Link to={`/blog/${b.slug}`} className="font-bold text-[#E83E8C] group-hover:underline flex items-center gap-1">
                    <span>Read More</span>
