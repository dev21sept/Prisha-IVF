...
import { Microscope, ArrowRight, Check, Search } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 space-y-12">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white p-4 rounded-2xl shadow-xs border border-slate-100">
          <div className="flex items-center gap-2 w-full sm:w-80 px-3.5 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus-within:bg-white focus-within:border-[#0E5D94] transition-colors">
            ...
            <input type="text" placeholder="Search treatment (e.g. ICSI, Freezing)..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full text-xs sm:text-sm bg-transparent outline-none text-slate-800"/>
          </div>

          <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto overflow-x-auto pb-1 sm:pb-0">
            {categories.map((cat) => (<button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all shrink-0 ${selectedCategory === cat
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredServices.map((srv) => (<div key={srv.slug} className="bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col justify-between overflow-hidden group hover:border-sky-200">
              <div>
                ...
                <div className="p-6">
                  ...
                  <div className="space-y-1.5 mb-2 text-xs text-slate-700">
                    ...
                    {srv.benefits.slice(0, 3).map((b, i) => (<div key={i} className="flex items-center gap-2">
              ...
              <div className="p-6 pt-0 border-t border-slate-100 flex items-center justify-between">
                <Link to={`/services/${srv.slug}`} className="text-xs font-bold text-[#0E5D94] group-hover:text-[#E83E8C] flex items-center gap-1.5 transition-colors">
                  <span>Explore Procedure Details</span>
                  <ArrowRight className="w-3.5 h-3.5 group-hover:translate-x-1 transition-transform"/>
                </Link>

                <Link to={`/appointment?service=${encodeURIComponent(srv.name)}`} className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-sky-50 text-[#0E5D94] hover:bg-[#0E5D94] hover:text-white transition-colors">
                  Book Slot
                </Link>
              </div>
            </div>))}
        ...
        {filteredServices.length === 0 && (<div className="text-center py-16 bg-white rounded-2xl border border-slate-100">
