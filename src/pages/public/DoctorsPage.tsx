...
import { Award, Calendar, ChevronRight, GraduationCap, ShieldCheck } from 'lucide-react';
          ...
          {doctors.map((doc) => (
            ...
            >
              <div>
                ...
                <div className="p-6">
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-4">
                    {doc.specialization.slice(0, 3).map((spec, i) => (
                      <span key={i} className="px-2.5 py-0.5 rounded-md bg-sky-50 text-[11px] font-medium text-[#0E5D94] border border-sky-100">
                        {spec}
                      </span>
                    ))}
                  </div>
              ...
              <div className="p-6 pt-0 border-t border-slate-100 flex items-center justify-between gap-3">
                <Link
                  to={`/doctors/${doc.slug}`}
                  className="text-xs font-bold text-[#0E5D94] hover:text-[#E83E8C] flex items-center gap-1 transition-colors"
                >
                ...
                <Link
                  to={`/appointment?doctor=${encodeURIComponent(doc.name)}`}
                  className="px-4 py-2 rounded-lg text-xs font-bold text-white bg-[#0E5D94] hover:bg-[#0a4975] transition-colors flex items-center gap-1.5 shadow-xs"
                >
