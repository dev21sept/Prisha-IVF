...
import { Calendar, ChevronRight, GraduationCap, Clock } from 'lucide-react';
          ...
          <div className="lg:col-span-8 space-y-5">
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                {doctor.specialization.map((spec, i) => (<span key={i} className="px-3 py-1 rounded-lg bg-sky-50 text-xs font-semibold text-[#0E5D94] border border-sky-100">
                    {spec}
                  </span>))}
              </div>
            </div>
            ...
            <div className="pt-4 border-t border-slate-100 flex flex-wrap items-center gap-6">
              ...
              <Link to={`/appointment?doctor=${encodeURIComponent(doctor.name)}`} className="px-8 py-3.5 rounded-full text-sm font-bold text-white bg-gradient-to-r from-[#0E5D94] to-[#0A4D7A] hover:from-[#0a4975] hover:to-[#0E5D94] shadow-md transition-all flex items-center gap-2">
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-xs space-y-4">
              ...
              <ul className="space-y-3 text-xs">
                {otherDoctors.map((od) => (<li key={od.id}>
                    <Link to={`/doctors/${od.slug}`} className="flex items-center gap-3 p-2 rounded-lg hover:bg-sky-50 transition-colors">
