      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-8 space-y-10">
        ...
        <article className="bg-white rounded-3xl p-8 sm:p-12 border border-slate-100 shadow-sm space-y-6 text-slate-700 text-sm sm:text-base leading-relaxed">
          {blog.content.split('\n\n').map((paragraph, index) => {
            if (paragraph.startsWith('### ')) {
                return (<h3 key={index} className="text-xl sm:text-2xl font-bold text-slate-900 pt-4 pb-1 border-b border-slate-100">
                  {paragraph.replace('### ', '')}
                </h3>);
            }
            if (paragraph.startsWith('1. ') || paragraph.startsWith('- ')) {
                return (<div key={index} className="pl-4 space-y-1.5 text-slate-700 text-sm">
                  {paragraph.split('\n').map((li, i) => (<div key={i} className="flex items-start gap-2">
                      <span className="text-[#E83E8C] font-bold">•</span>
                      <span>{li.replace(/^[0-9]+\.\s+|^-\s+/, '')}</span>
                    </div>))}
                </div>);
            }
            return (<p key={index} className="text-slate-600 leading-relaxed">
                {paragraph}
              </p>);
        })}

          {/* Tags */}
          {blog.tags && blog.tags.length > 0 && (<div className="pt-8 border-t border-slate-100 flex flex-wrap items-center gap-2">
              <span className="text-xs font-bold text-slate-500 flex items-center gap-1 mr-2">
                <Tag className="w-3.5 h-3.5"/> Tags:
              </span>
              {blog.tags.map((tag, i) => (<span key={i} className="px-3 py-1 rounded-full bg-slate-100 text-xs font-semibold text-slate-600">
                  #{tag}
                </span>))}
            </div>)}
        </article>
        ...
        <div className="space-y-6 pt-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            {relatedBlogs.map((rb) => (<Link key={rb.id} to={`/blog/${rb.slug}`} className="bg-white rounded-xl overflow-hidden border border-slate-100 shadow-xs hover:shadow-md transition-all group p-4 flex flex-col justify-between">
                <div>
        ...
        <div className="pt-4 text-center">
          <Link to="/blog" className="inline-flex items-center gap-2 text-xs font-bold text-[#0E5D94] hover:underline">
