...
import { Save, CheckCircle, Home, Sparkles } from 'lucide-react';
    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          ...
          disabled={saving}
          className="px-6 py-2.5 rounded-xl text-xs font-bold text-white bg-gradient-to-r from-[#0E5D94] to-[#0A4D7A] hover:from-[#0a4975] hover:to-[#0E5D94] shadow-md transition-all flex items-center gap-2 self-start sm:self-auto disabled:opacity-50"
        >
      ...
      {savedSuccess && (
        <div className="flex items-center gap-2 p-4 rounded-xl bg-emerald-50 text-emerald-800 text-xs font-bold border border-emerald-200">
      ...
      <form onSubmit={handleSubmit} className="space-y-8 text-xs">
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <input
                ...
                className="w-full p-2.5 rounded-xl border border-slate-200 focus:border-[#0E5D94] outline-none text-sm font-semibold"
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full p-2.5 rounded-xl border border-slate-200 focus:border-[#0E5D94] outline-none leading-relaxed"
              />
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="p-3.5 rounded-xl bg-slate-50 space-y-2">
              ...
              <input
                ...
                placeholder="12,500+"
                className="w-full p-2 rounded-lg border border-slate-200 text-sm font-extrabold text-[#0E5D94]"
              />
              <input
                type="text"
                value={form.stat1Label || ''}
                onChange={(e) => setForm({ ...form, stat1Label: e.target.value })}
                placeholder="Parenthood Deliveries"
                className="w-full p-2 rounded-lg border border-slate-200 text-[11px]"
              />
            </div>
            ...
            <div className="p-3.5 rounded-xl bg-slate-50 space-y-2">
              ...
              <input
                ...
                placeholder="78.4%"
                className="w-full p-2 rounded-lg border border-slate-200 text-sm font-extrabold text-[#E83E8C]"
              />
              <input
                type="text"
                value={form.stat2Label || ''}
                onChange={(e) => setForm({ ...form, stat2Label: e.target.value })}
                placeholder="Cumulative Success"
                className="w-full p-2 rounded-lg border border-slate-200 text-[11px]"
              />
            </div>
            ...
            <div className="p-3.5 rounded-xl bg-slate-50 space-y-2">
              ...
              <input
                ...
                onChange={(e) => setForm({ ...form, stat4Value: e.target.value })}
                placeholder="100%"
                className="w-full p-2 rounded-lg border border-slate-200 text-sm font-extrabold text-[#E83E8C]"
              />
              <input
        ...
        <div className="flex justify-end pt-4">
          <button
            type="submit"
            disabled={saving}
            className="px-8 py-3 rounded-xl text-xs font-bold text-white bg-[#0E5D94] hover:bg-[#0a4975] shadow-md transition-all disabled:opacity-50"
          >
