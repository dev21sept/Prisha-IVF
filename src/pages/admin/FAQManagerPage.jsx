...
export const FAQManagerPage = () => {
    ...
    return (<div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="space-y-3">
        {faqs.map((faq) => (<div key={faq.id} className="bg-white rounded-2xl border border-slate-200 shadow-xs p-5 flex items-start justify-between gap-4 hover:border-sky-200 transition-all">
            ...
            <div className="flex items-center gap-2 shrink-0">
              <button onClick={() => handleOpenEdit(faq)} className="p-1.5 rounded-lg bg-sky-50 text-[#0E5D94] hover:bg-sky-100 font-bold" title="Edit FAQ">
                <Edit2 className="w-3.5 h-3.5"/>
              </button>
              <button onClick={() => handleDelete(faq.id, faq.question)} className="p-1.5 rounded-lg text-slate-400 hover:text-red-600 hover:bg-red-50" title="Delete FAQ">
      ...
      {isModalOpen && editingFaq && (<div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 sm:p-8 shadow-2xl border border-slate-200 space-y-5 animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleSave} className="space-y-3.5 text-xs">
              <div>
                ...
                <select value={editingFaq.category || 'General IVF'} onChange={(e) => setEditingFaq({ ...editingFaq, category: e.target.value })} className="w-full p-2.5 rounded-xl border border-slate-200 outline-none bg-white">
              ...
              <div>
                ...
                <input type="text" required placeholder="e.g. What is the difference between IUI and IVF?" value={editingFaq.question || ''} onChange={(e) => setEditingFaq({ ...editingFaq, question: e.target.value })} className="w-full p-2.5 rounded-xl border border-slate-200 outline-none font-semibold text-slate-800"/>
              </div>

              <div>
                <label className="block font-bold text-slate-700 mb-1">Medical Answer *</label>
                <textarea rows={4} required placeholder="Explain clearly in simple, reassuring language..." value={editingFaq.answer || ''} onChange={(e) => setEditingFaq({ ...editingFaq, answer: e.target.value })} className="w-full p-2.5 rounded-xl border border-slate-200 outline-none leading-relaxed"/>
              </div>
