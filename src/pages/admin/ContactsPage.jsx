    ...
    return (<div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-2 w-full md:w-80 px-3.5 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus-within:bg-white focus-within:border-[#0E5D94] transition-colors">
          ...
          <input type="text" placeholder="Search inquiries by name, phone, subject..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full text-xs sm:text-sm bg-transparent outline-none text-slate-800"/>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs text-slate-700">
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredContacts.map((c) => (<tr key={c.id} className="hover:bg-slate-50/60 transition-colors">
                  ...
                  <td className="p-4">
                    <select value={c.status} onChange={(e) => updateContactStatus(c.id, e.target.value)} className={`text-xs font-bold px-2.5 py-1 rounded-full border outline-none cursor-pointer ${getStatusBadge(c.status)}`}>
                  ...
                  <td className="p-4 text-right space-x-2">
                    <button onClick={() => handleOpen(c)} className="p-1.5 text-slate-500 hover:text-[#0E5D94] hover:bg-sky-50 rounded-lg transition-colors" title="Read Message">
                      <Eye className="w-4 h-4"/>
                    </button>
                    <button onClick={() => handleDelete(c.id, c.name)} className="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Delete">
        ...
        {filteredContacts.length === 0 && (<div className="text-center py-16">
      ...
      {/* Message Modal */}
      {selectedMsg && (<div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 sm:p-8 shadow-2xl border border-slate-200 space-y-6 animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-start border-b border-slate-100 pb-4">
              ...
              <button onClick={() => setSelectedMsg(null)} className="text-slate-400 hover:text-slate-600 font-bold text-lg">
                ✕
              </button>
            </div>
            ...
            <div className="space-y-2 text-xs">
              ...
              <textarea rows={3} placeholder="e.g. Called patient on phone and answered IVF package questions." value={replyNotes} onChange={(e) => setReplyNotes(e.target.value)} className="w-full p-3 rounded-xl border border-slate-200 focus:border-[#0E5D94] outline-none"/>
              <div className="flex justify-end">
                <button onClick={handleSaveReply} className="px-4 py-1.5 rounded-lg text-xs font-bold text-white bg-gradient-to-r from-[#0E5D94] to-[#0A4D7A] hover:from-[#0a4975] hover:to-[#0E5D94] transition-colors">
                  Save Reply Status
                </button>
              </div>
            </div>
