...
export const AdminLayout: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col lg:flex-row text-slate-800">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed lg:sticky top-0 h-screen w-72 bg-[#0f1e2c] text-slate-300 flex flex-col justify-between z-40 transition-transform duration-300 ${
              ...
              return (
                ...
                >
                  ...
                  {item.badge !== undefined && item.badge > 0 && (
                    <span className="px-2 py-0.5 rounded-full text-[10px] font-extrabold bg-[#E83E8C] text-white shadow-xs">
                      {item.badge}
                    </span>
                  )}
                </Link>
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-900/60 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2 pt-1 text-xs">
            <Link
              to="/"
              target="_blank"
              className="px-2.5 py-2 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white transition-colors flex items-center justify-center gap-1.5 font-semibold text-[11px]"
            >
            ...
            <button
              ...
              className="px-2.5 py-2 rounded-lg bg-red-950/40 hover:bg-red-900/60 text-red-300 hover:text-red-100 transition-colors flex items-center justify-center gap-1.5 font-semibold text-[11px]"
            >
        ...
        <header className="bg-white border-b border-slate-200 px-6 py-3.5 hidden lg:flex justify-between items-center shadow-xs">
          ...
          <div className="flex items-center gap-4 text-xs font-semibold">
            <Link
              to="/"
              target="_blank"
              className="px-3 py-1.5 rounded-lg bg-sky-50 text-[#0E5D94] hover:bg-sky-100 border border-sky-100 transition-colors flex items-center gap-1.5"
            >
