...
export const Header: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      <div
        className={`text-xs py-2 px-4 sm:px-8 border-b hidden md:block transition-colors duration-300 ${
          isTransparent
      ...
      >
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-1.5 text-sky-100">
              ...
              <a
                ...
                className="font-semibold hover:text-[#E83E8C] transition-colors"
              >
                {settings.phone}
              </a>
            </div>
          ...
          <div className="flex items-center gap-4">
            ...
            {currentUser ? (
              <Link to="/admin" className="flex items-center gap-1 text-[#E83E8C] font-semibold hover:underline">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 py-3.5 flex justify-between items-center">
        ...
        <Link to="/" className="flex items-center gap-3 group focus:outline-none">
          <div
            className={`transition-all duration-300 rounded-2xl flex items-center ${
              isTransparent
            ...
            if (link.hasDropdown) {
              return (
                ...
                >
                  ...
                  >
                    ...
                    <ChevronDown
                      className={`w-4 h-4 transition-transform duration-200 ${
                        servicesDropdownOpen ? 'rotate-180 text-[#E83E8C]' : ''
                      }`}
                    />
                  </Link>
                  ...
                  {/* Dropdown Menu */}
                  <div
                    className={`absolute top-full left-0 w-72 bg-white rounded-xl shadow-xl border border-slate-100 p-2.5 transition-all duration-200 z-50 text-slate-800 ${
                  ...
                  >
                    ...
                    <div className="mt-1">
                      {featuredServices.map((srv) => (
                        <Link
                          ...
                          className="block px-3 py-2 text-sm rounded-lg hover:bg-sky-50 text-slate-700 hover:text-[#0E5D94] transition-colors"
                    ...
                    <div className="pt-2 mt-1 border-t border-slate-100">
                      <Link
                        to="/services"
                        className="block px-3 py-1.5 text-xs text-center font-bold text-[#E83E8C] hover:bg-pink-50 rounded-lg transition-colors"
                        onClick={() => setServicesDropdownOpen(false)}
                      >
        ...
        <div className="hidden sm:flex items-center gap-3">
          <a
            href={`tel:${settings.phone.replace(/\s+/g, '')}`}
            className={`flex lg:hidden xl:flex items-center gap-2 px-3.5 py-2 text-xs font-bold rounded-xl transition-all ${
              isTransparent
          ...
          <Link
            to="/appointment"
            className={`inline-flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-bold text-white transition-all duration-300 group shadow-md hover:shadow-lg ${
              isTransparent
        ...
        <button
          ...
          className={`lg:hidden p-2 rounded-lg transition-colors focus:outline-none ${
            isTransparent
              ...
              : 'text-slate-700 hover:text-[#0E5D94] hover:bg-slate-100'
          }`}
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden bg-white border-t border-slate-100 px-6 py-5 shadow-2xl animate-in slide-in-from-top-4 duration-300 text-slate-800">
          <nav className="flex flex-col gap-3 font-medium">
            ...
            <div className="pt-4 border-t border-slate-100 flex flex-col gap-3">
              <Link
                to="/appointment"
                onClick={() => setMobileMenuOpen(false)}
                className="w-full text-center py-3 rounded-full text-sm font-bold text-white bg-gradient-to-r from-[#0E5D94] to-[#0A4D7A] shadow-md"
              >
                Book An Appointment
              </Link>
